...
状態: **Verified (2026-09-23 phaseF6 20/20・run-all 18スイート297テスト ALL GREEN。機体シルエットの飽和 5.3%)** ／ 親: [SPEC-09](09-realism-gameplay-policy.md) R3 + R4 ／ 前提: F2（GradePass / Look）・F4（高さ→法線の生成器）Verified ／ 外部素材: 不要
...
- **サイト(PC)の仕上げ**: GradePass を最終パスにし、ACES・仕上げ・sRGB 化までを 1 パスで行う（`encode=1`）。
  画面へ直接描くとき three が `RRTAndODTFit` 等を前置するため、シェーダ関数名は `vg` 接頭辞で衝突を回避
- **粒状ノイズ**は知覚空間（≒sRGB）で加えてから線形に戻す。線形空間で加えると暗部だけが大きく持ち上がり荒れるため
- 外板の粗さマップは平均が約 1 になるよう較正（`roughness .34` × マップ ≒ .32）。UV は三角形ごとの箱投影（密度比 1.00〜1.20）
- ノズル光の HDR（×1.8、F2 から移管）はプルーム化で輪郭を覆わなくなったため復活。エンジン光は scale 4 → 2
- 被弾の外板点灯は実時間で減衰し、ゲーム開始時にも必ず消灯（点灯中に大破→リトライで橙が残る不具合を回避）
- 近傍ダストの移動はシミュレーション時間（ヒットストップ中は止まる）
