...
### 3.5 実プレイFBによる修正（2026-07-20）

- **カメラ寄り(camOff)の蓄積バグ**: `camera.position += gm` を毎フレーム後付け加算していたため、
  lerp係数0.05に対し均衡点が `目標 + gm/0.05`(=20倍)まで発散。SURVIVALの camOff{-0.6,+0.4} が
  実質 x-12/y+8 のカメラ移動となり、背景が大きく右下へズレて機体が画面外(特にSP)へ出ていた。
  → **camOffを補間の目標に折り込む**方式へ修正（`stepPageCamera()`に集約）。
  実測ドリフト0.00、機体NDCはPC(0,-0.12)/SP縦(0.01,-0.12)で画面内。
