...
> 置換（SPEC-16 注記）: 閾値・強度は SPEC-09b で HDR 基準（閾値 1.0・強度 .45）に改訂。パス順は Render → Bloom → Grade(ACES) → HUD → Afterimage。
...
| パラメータ | 初期値 | 調整範囲 | 備考 |
|---|---|---|---|
| threshold | 0.62 (初期0.55→実プレイFBで調整) | 0.4〜0.8 | 低すぎると岩・コックピット金属面まで光る(BLM-03) |
...
### 3.5 実プレイフィードバックによる調整（2026-07-20）

- 「スタート時に青い光が全体にかかり見えにくい」→ コックピットの発光素材が原因のため
  トリム 0x00f0ff→0x0d8ea4 / リング類 0x00f0ff→0x17aec4 / 内照明 intensity 6→3 に減光し、
  threshold を 0.55→0.62 に引き上げ（コア・トレーサー等の純色発光は維持）
- 「初弾だけラグ」→ プールFXが `visible:false` 待機のため初回表示時にシェーダコンパイルが
  走っていた。`build()` 末尾でFX一時表示+`composer.render()` 1回の**ウォームアップ**を追加
  （実測: 初弾フレーム 2.9ms・初爆発 4.7ms に改善）
